...
class AuthViewsTestCase(unittest.TestCase):
    ...
    def test_organizer_has_not_paid(self):
        ...
        with self.client:
            ...
            self.assertTrue("Please complete your signup process by purchasing your subscription below." \
    ...
    def test_login_valid_input(self):
        ...
        self.assertTrue('href="/auth/request_password_reset"' in response.get_data(as_text=True))
        ...
        for login in ViewFunctionTestData.VALID_LOGINS:
            with self.subTest(login=login):
                ...
                if login["role_name"] == "Event Organizer":
                    ...
                    self.assertTrue("Organizer Profile" in response.get_data(as_text=True))
                    self.assertTrue("Manage Events" in response.get_data(as_text=True))
                else:
                    self.assertTrue("Sponsor Profile" in response.get_data(as_text=True))
                    self.assertTrue("Manage Sponsorships" in response.get_data(as_text=True))
    ...
    def test_login_invalid_input(self):
        ...
        for login in ViewFunctionTestData.INVALID_LOGINS:
            with self.subTest(login=login):
                ...
                self.assertTrue(login["error_message"] in response.get_data(as_text=True))
    ...
    def test_register_valid_input(self):
        ...
        for registration in ViewFunctionTestData.VALID_REGISTRATIONS:
            with self.subTest(registration=registration):
                ...
                self.assertEqual(response.status_code, 200)
                self.assertTrue(registration["message"] in response.get_data(as_text=True))
    ...
    def test_register_invalid_input(self):
        ...
        role = Role.query.filter_by(name="Event Organizer").first()
        user = TestModelFactory.create_user(password="password", company="Amazon", email="dave@amazon.com")
        user.role = role
        db.session.add(user)
        db.session.commit()
        ...
        for registration in ViewFunctionTestData.INVALID_REGISTRATIONS:
                with self.subTest(registration=registration):
                    ...
                    self.assertTrue(registration["error_message"] in response.get_data(as_text=True))
    ...
    def test_change_password_valid_input(self):
        ...
        with self.client:
            ...
            #send POST request
            response = self.client.post("/settings/change_password", data={
                "old_password": ViewFunctionTestData.VALID_CHANGE_PASSWORD_DATA["old_password"],
                "new_password": ViewFunctionTestData.VALID_CHANGE_PASSWORD_DATA["new_password"],
                "confirm_password": ViewFunctionTestData.VALID_CHANGE_PASSWORD_DATA["confirm_password"]
            }, follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertTrue(ViewFunctionTestData.VALID_CHANGE_PASSWORD_DATA["message"] in response.get_data(as_text=True))

        self.assertTrue(user.verify_password(ViewFunctionTestData.VALID_CHANGE_PASSWORD_DATA["new_password"]))        
    ...
    def test_close_account_valid_input(self):
        ...
        with self.client:
            ...
            self.assertTrue("Please enter &#34;CLOSE&#34; in the field below to confirm the closing of your account" \
                in response.get_data(as_text=True)
            )

            #Send post request
            response = self.client.post("/settings/close_account", data={
                "confirm": "CLOSE"
            }, follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertTrue("Your account has been closed."
                in response.get_data(as_text=True)
            )
            ...
            self.assertIsNone(User.query.get(1)) #user should be deleted from the database
        ...
        with self.client:
            ...
            for data in ViewFunctionTestData.INVALID_CLOSE_ACCOUNT_DATA:
                with self.subTest(data=data):
                    ...
                    self.assertTrue("Please enter &#34;CLOSE&#34;" in response.get_data(as_text=True))
    ...
    def test_organizer_permissions(self):
        ...
        with self.client:
            ...
            for route in ViewFunctionTestData.SPONSOR_ROUTES + ViewFunctionTestData.ADMIN_ROUTES:
    ...
    def test_sponsor_permissions(self):
        ...
        with self.client:
            ...
            for route in ViewFunctionTestData.ORGANIZER_ROUTES + ViewFunctionTestData.ADMIN_ROUTES:
                with self.subTest(route=route):
                    response = self.client.get(route, follow_redirects=True)
                    self.assertEqual(response.status_code, 403)
                    self.assertTrue("Forbidden" in response.get_data(as_text=True))
