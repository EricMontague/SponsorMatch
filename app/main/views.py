...
g, current_app, session, jsonify, make_response
from flask_login import login_required, current_user
from . import main
from .forms import CreateEventForm, EventDetailsForm, EventPackagesForm, \
UploadVideoForm, RemoveVideoForm, MultipleImageForm, DemographicsForm, \
UploadImageForm, RemoveImageForm, EditProfileForm, EditProfileAdminForm, \
ContactForm, AdvancedSearchForm, SearchForm, DropdownForm
...
def advanced_search():
    ...
    if form.validate_on_submit():
        ...
        query = Event.query.join(Venue, Venue.id == Event.venue_id) \
            .filter(Venue.state == state) \
            .filter(func.Date(Event.start_datetime) >= form.start_date.data, 
        ...
        #user filled out the form with complete specificity
            ...
            pagination = query.filter(Venue.city == form.city.data) \
                .filter(Event.event_category_id == form.category.data).paginate(
                    page, per_page=current_app.config["EVENTS_PER_PAGE"], error_out=False
                )
        events = pagination.items
        prev_url = None
...
def search():
    ...
    if page > 1:
        prev_url = url_for(
            "main.search", query=g.search_form.query.data, page=page - 1
        )
    next_url = None
...
def user_profile(company):
    ...
    if user.can(Permission.CREATE_EVENT) or user.is_administrator():
        ...
        if past == 1:
            ...
            query = Event.query.join(Sponsorship, Sponsorship.event_id == Event.id) \
                .filter(Event.is_past() == True) \
                .filter(Sponsorship.sponsor_id == user.id, Sponsorship.is_pending() == False)
...
def user_sponsorships(id, status):
    ...
    if status == SponsorshipStatus.CURRENT:
        events = [sponsorship.event for sponsorship in user.sponsorships if not sponsorship.is_pending() and sponsorship.is_current()]
    elif status == SponsorshipStatus.PAST:
        events = [sponsorship.event for sponsorship in user.sponsorships if not sponsorship.is_pending() and sponsorship.is_past()]
    else:
        abort(404)
    return render_template("_events.html", events=events)
...
def demographics(id):
    ...
    if form.validate_on_submit():
        if form.males.data + form.females.data != 100:
            ...
            event.attendees = DemographicsForm.choice_value(form.attendees.data, "PEOPLE_RANGES")
    ...
    if event.attendees:
        form.attendees.data = DemographicsForm.choice_id(event.attendees, "PEOPLE_RANGES")
    else:
...
def packages(id):
    ...
    if form.validate_on_submit():
        package = Package(
            ...
            audience=EventPackagesForm.choice_value(form.audience.data, "PEOPLE_RANGES"),
            description=form.description.data,
            available_packages=form.available_packages.data,
            package_type=EventPackagesForm.choice_value(form.package_type.data, "PACKAGE_TYPES"),
            event=event
        )
        db.session.commit()
        return redirect(url_for("main.packages", id=id))
    return render_template(
        "main/events/packages.html", 
        form=form, 
        event=event,
        packages=packages
    )
...
@main.route(
    "/events/<int:event_id>/packages/<int:package_id>/edit", 
    methods=["GET", "POST"]
)
@login_required
@permission_required(Permission.CREATE_EVENT)
def edit_package(event_id, package_id):
    ...
    if form.validate_on_submit():
        ...
        package.audience = EventPackagesForm.choice_value(form.audience.data, "PEOPLE_RANGES")
        package.description = form.description.data
        package.available_packages = form.available_packages.data
        package.package_type = EventPackagesForm.choice_value(form.package_type.data, "PACKAGE_TYPES")
        db.session.commit()
    ...
    form.package_type.data = EventPackagesForm.choice_id(package.package_type, "PACKAGE_TYPES")
    return render_template(
        "main/events/packages.html", 
        form=form, 
        event=event,
...
@main.route("/events/<int:event_id>/packages/<int:package_id>/delete", 
           methods=["POST"])
@login_required
@permission_required(Permission.CREATE_EVENT)
...
def add_video(id):
    ...
    if upload_video_form.validate_on_submit():
        video = Video(
            url=UploadVideoForm.parse_url(upload_video_form.video_url.data), 
            event=event
        )
...
def media(id):
    ...
    if not current_user.is_organizer(event) and not current_user.is_administrator():
        return redirect(url_for("main.index"))
    return render_template("main/events/media.html",
...
@main.route("/events/<int:event_id>/videos/<int:video_id>/delete", 
           methods=["POST"])
@login_required
@permission_required(Permission.CREATE_EVENT)
...
def publish(id):
    ...
    """
    event = Event.query.get_or_404(id)
    if (not current_user.is_organizer(event) and not current_user.is_administrator()) or event.has_ended():
        return redirect(url_for("main.index"))
...
def event(id):
    ...
    packages = event.packages.all()
    if form.validate_on_submit():
        send_email(organizer.email, f"Event Inquiry - {form.subject.data}",
                  "main/email/contact_organizer", organizer=organizer,
                  form=form, event=event)
        flash("Your email was sent to the event organizer.")
        return redirect(url_for("main.event", id=id))
    return render_template(
        "main/events/event.html", event=event,
        venue=venue, organizer=organizer, packages=packages, 
        form=form, date_format=date_format, image_path=image_path,
        time_format=time_format, other_media=other_media)
...
def saved_events():
    ...
    return render_template("main/events/saved_events.html", events=events, endpoint=endpoint)
...
def delete_saved_event(id):
    ...
    if current_user.has_saved(event):
        ...
        db.session.commit()
        return jsonify({"message": "The event was removed from your saved events list."})
    return redirect(url_for("main.index"))
...
def manage_sponsorships(status):
    ...
    user = current_user._get_current_object()
    if status == SponsorshipStatus.CURRENT:
        sponsorships = [sponsorship for sponsorship in user.sponsorships if not sponsorship.is_pending() and sponsorship.is_current()]
    elif status == SponsorshipStatus.PAST:
        sponsorships = [sponsorship for sponsorship in user.sponsorships if not sponsorship.is_pending() and sponsorship.is_past()]
    else:
        sponsorships = [sponsorship for sponsorship in user.sponsorships if not sponsorship.is_pending()]
...
def charge(id, amount):
    ...
    sponsorships = Sponsorship.query.filter(
            ...
            Sponsorship.event_id == event.id).all()
    ...
    try:
        ...
        for sponsorship in sponsorships:
            ...
            if sponsorship.package.available_packages - sponsorship.package.num_purchased < 0: #out of stock
                db.session.rollback()
                flash("The package you attempted to purchase is sold out.")
                return redirect(url_for("main.event", id=event.id))
        db.session.commit()
        send_email(user.email, "Your Recent Purchase",
              "main/email/purchase", user=user,
              sponsorships=sponsorships, amount=amount)
    ...
    #refactor needed. Maybe I can make a decorator to handle stripe errors?
      ...
      # Too many requests made to the API too quickly
        db.session.rollback()
        flash("Sorry, we are experiencing high traffic volumes. Please wait 30 seconds before retrying your purchase.")
        return redirect(url_for("main.event", id=event.id))
        ...
        db.session.rollback()
        flash("We are having issues connecting to the Stripe API. We will try to resolve this issue as soon as possible. Please try to make your purchase again later.")
        return redirect(url_for("main.event", id=event.id))
        ...
        db.session.rollback()
        flash("We are having issues accessing the Stripe API. We will try to resolve this issue as soon as possible. Please try to make your purchase again later.")
        return redirect(url_for("main.event", id=event.id))
...
def purchase(id):
    ...
    if sponsorships == []: #user has no pending deals for this event
        return redirect(url_for("main.index"))
    flash("Please note: Navigating away from or refreshing this page will cancel your purchase.")
    return render_template(
...
def cancel_purchase(id):
    ...
    event = Event.query.get_or_404(id)
    user = current_user._get_current_object()
    sponsorships = [sponsorship for sponsorship in user.sponsorships if sponsorship.event_id == event.id and sponsorship.is_pending()] 
    if sponsorships != []:
        for sponsorship in sponsorships:
            try:
