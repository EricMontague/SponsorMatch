...
from app import db
from app.models import User, Role, Event, EventCategory, \
EventType, Venue, Package, ImageType, Sponsorship


class FakeDataGenerator:
    """Class to generate fake data for the application."""

    def __init__(self, num_users, num_events, packages_per_event=4, sponsors_per_event=3):
      self.faker = Faker()
    ...
    def add_users(self):
        ...
        while i < self.num_users:
            role = Role.query.filter_by(
                name=random.choice(["Event Organizer", "Sponsor"])).first()
    ...
    def add_events(self):
        ...
        for index in range(self.num_events):
            ...
            venue = Venue(name=self.faker.company(),
                         address=self.faker.street_address(),
                         city=self.faker.city(),
                         state=self.faker.state(),
                         zip_code=self.faker.zipcode())
            start_date = self.faker.date_between(start_date=datetime.now() + timedelta(days=1), 
                                          end_date="+30d")
            ...
            event = Event(title=self.faker.company() + random.choice([" Party", " Gala"]),
    ...
    def add_packages(self):
        ...
        for count in range(1, event_count + 1):
            ...
            for num in range(self.packages_per_event):
                package = Package(name=self.faker.color_name() + " Package",
